...
form.addEventListener("submit", async (e) => {
  ...
  const dropboxLink = document.getElementById("dropbox_link").value.trim();
  const mediaKitLink = document.getElementById("media_kit_link").value.trim();
  const quotedFee = document.getElementById("quoted_fee").value.trim();
  ...
  if (queuedFiles.length === 0 && !driveLink && !dropboxLink && !mediaKitLink) {
    return setStatus("error", "Add screenshots OR a Drive folder URL OR a Dropbox folder URL OR a media kit URL");
  }
  ...
  if (dropboxLink) formData.append("dropbox_link", dropboxLink);
  if (mediaKitLink) formData.append("media_kit_link", mediaKitLink);
  if (quotedFee) formData.append("quoted_fee", quotedFee);
